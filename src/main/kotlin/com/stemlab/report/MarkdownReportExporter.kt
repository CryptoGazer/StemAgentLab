...
object MarkdownReportExporter {
    ...
    fun export(
        ...
        outputPath: String = "projects/default/reports/report-${result.runId}.md",
        narrative: String? = null
    ): String {
        val md = buildReport(result, narrative)
    ...
    fun buildReport(result: EvolutionResult, narrative: String? = null): String = buildString {
        ...
        appendLine("---")
        appendLine()
        appendLine("## Human Summary")
        appendLine()
        appendLine(narrative?.takeIf { it.isNotBlank() } ?: fallbackNarrative(result))
        appendLine()
        appendLine("---")
        appendLine()
    ...
    private fun fallbackNarrative(result: EvolutionResult): String {
        val selected = result.selectedCandidate
        return if (selected == null) {
            "This run evaluated the baseline agent and candidate configurations for `${result.domain}`, " +
                "but no candidate improved on the baseline score of ${"%.3f".format(result.baselineScore)}. " +
                "The safest outcome is to keep the baseline configuration for this project."
        } else {
            "${selected.config.name} was selected for `${result.domain}` after outperforming the baseline. " +
                "The score moved from ${"%.3f".format(result.baselineScore)} to ${"%.3f".format(selected.score)}, " +
                "with an improvement of ${"%.1f".format(result.improvementPercent)}%. " +
                "The selected configuration should be treated as the frozen specialized agent for this run."
        }
    }
}
